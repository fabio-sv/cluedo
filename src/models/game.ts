import { characters, rooms, TABLE_IDX, weapons } from "../data/index.ts";
...
export class Game implements IGame {
  ...
  constructor(playerNames: string[]) {
    ...
    this.players.forEach((player) => {
      player.cards = draw(this.table, this.cardsPerHand);
    });
  ...
  playRound(): Outcome | undefined {
    ...
    if (currentSolution) {
      const areTheyCorrect =
        currentSolution.character === this.solution.character &&
        currentSolution.weapon === this.solution.weapon &&
        currentSolution.room === this.solution.room;

      this.debug(`${current.idx}: CLUEDO!`);
      this.debug(
        `${current.idx}: ${current.name} claims the murder was committed by ${currentSolution.character} with ${currentSolution.weapon} in the ${currentSolution.room}...`,
      );
      this.debug(
        `${current.idx}: Are they correct? They are${
          areTheyCorrect ? "!" : " not!"
        }`,
      );
    ...
    for (
      let i = (current.idx + 1) % this.players.length;
      i !== current.idx;
      ...
      if (player.idx != current.idx) {
        ...
        if (response) {
          this.debug(
            `${current.idx}: ${player.name} can help <${response}>. Adding to ${current.name}'s knowledge.`,
          );
    ...
    if (!shown) {
      this.debug(
        `${current.idx}: No one can help. Adding ${
          Object.values(currentGuess).join(", ")
        } to ${current.name}'s knowledge.`,
      );

      const updateAtoms: CluedoAtom[] = Object.values(currentGuess).map<
        CluedoAtom
      >((guessAtom) => ({
        name: guessAtom,
